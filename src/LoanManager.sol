    ...
    Initializable
{
    ...
    function repay(uint256 loanId) public override {
        ...
        assetManager.depositFrom(loan.token, loan.repayAmount, msg.sender, address(lendingManager));
    ...
    function allocateFundOnLoan(uint256 loanId, uint256 termId, uint256 assets) public override onlyLender {
        ...
        require(
            loan.sharesAllocated + sharesRequired <= loan.sharesRequired, "StormbitLoanManager: loan amount exceeded"
        );
    ...
    function _calculateSharesRequired(address token, uint256 amount) private view returns (uint256) {
        // get the vault token
        address vaultToken = assetManager.getVaultToken(token);
        // convert amount to shares
    ...
    function getTermAllocatedSharesOnLoan(uint256 loanId, uint256 termId, address vaultToken)
        public
        view
        override
        returns (uint256)
