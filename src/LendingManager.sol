    ...
    ILendingManager
{
    ...
    function depositToTerm(
        ...
        if (lastDepositTime[msg.sender][termId] > 0) {
            ...
            uint256 userProfit = _calculateUserProfit(
                termId,
                lastDepositShares,
                token
            );
    ...
    function withdrawFromTerm(
        ...
        uint256 redeemShares = userProfit +
            shares +
            _unclaimProfit[msg.sender][vaultToken];

        uint256 newProfit = term.termBalances[vaultToken].profit.latest() -
            (userProfit + _unclaimProfit[msg.sender][vaultToken]);
        ...
        term.userSharesCheckpoints[msg.sender][vaultToken].push(
            clock(),
            SafeCast.toUint224(totalDelegatedShares - shares)
        );
    ...
    function _calculateUserProfit(
        ...
        uint256 currentTimeShares = term
            .termBalances[vaultToken]
            .shares
            .latest();
        uint256 lastTimeProfit = term
            .termBalances[vaultToken]
            .profit
            .upperLookupRecent(
                SafeCast.toUint32(lastDepositTime[msg.sender][termId])
